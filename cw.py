...
class cw:
   ...
   def calculate(self, days, data, eta, sigma_format):
       ...
       #sparse version 
       mu    = sparse.csc_matrix(np.zeros((nF,1)))
       sigma = sparse.eye(nF)
       ...
       for d in xrange(days):
           ...
           for i in xrange(nEx):
               ...
               if M < phi*math.sqrt(V):
                   alpha = (-M*psi + math.sqrt(M**2 * phi**4/4 + V*phi**2*xi)) / (V*xi)
                 
                   #if isreal(alpha) & ~isnan(alpha) & ~isinfinite(alpha)
                   if np.isfinite(alpha):
                       sqrtU = (-alpha*V*phi + math.sqrt(alpha**2*V**2*phi**2 + 4*V)) / 2
